...
void RuntimeLifecycle::AdvanceGeneration() noexcept {
  if (_generation == std::numeric_limits<std::uint64_t>::max()) {
    // Zero is never handed to a scene component, so wrapping to one cannot be
    // mistaken for an uninitialised generation.
    _generation = 1;
  } else {
    ++_generation;
  }
...
bool RuntimeLifecycle::Activate(std::uint64_t generation) noexcept {
  if (_phase != RuntimePhase::Preparing || generation != _generation) {
    return false;
  }
...
void RuntimeLifecycle::Suspend() noexcept {
  if (_phase == RuntimePhase::Active) {
    _phase = RuntimePhase::Suspended;
  }
}

void RuntimeLifecycle::Resume() noexcept {
  if (_phase == RuntimePhase::Suspended) {
    _phase = RuntimePhase::Active;
  }
}
...
bool RuntimeLifecycle::CompleteRetirement() noexcept {
  if (_phase != RuntimePhase::Retiring || _renderDepth != 0) {
    return false;
  }
