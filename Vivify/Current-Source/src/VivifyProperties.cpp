...
void UpdateDynamicGI() {
  using UpdateEnvironmentFn = void (*)();
  static UpdateEnvironmentFn updateEnvironment = reinterpret_cast<UpdateEnvironmentFn>(
      il2cpp_functions::resolve_icall("UnityEngine.DynamicGI::UpdateEnvironment"));
  static bool warned = false;
  if (updateEnvironment != nullptr) {
    updateEnvironment();
  } else if (!warned) {
    warned = true;
    PaperLogger.warn("Vivify could not resolve UnityEngine.DynamicGI::UpdateEnvironment; continuing without a GI refresh");
  }
}
}
...
template <typename T>
using SettingsSetter = void (*)(T);

template <typename T>
using SettingsGetter = T (*)();

template <typename T>
SettingsSetter<T> ResolveSettingsSetter(char const* icall) {
  static std::unordered_map<std::string_view, SettingsSetter<T>> cache;
  auto [it, inserted] = cache.try_emplace(icall, nullptr);
  if (inserted) {
    it->second = reinterpret_cast<SettingsSetter<T>>(il2cpp_functions::resolve_icall(icall));
    if (it->second == nullptr) {
      PaperLogger.warn("Vivify could not resolve rendering setting setter '{}'", icall);
    }
  }
  return it->second;
}
...
template <typename T>
SettingsGetter<T> ResolveSettingsGetter(char const* icall) {
  static std::unordered_map<std::string_view, SettingsGetter<T>> cache;
  auto [it, inserted] = cache.try_emplace(icall, nullptr);
  if (inserted) {
    it->second = reinterpret_cast<SettingsGetter<T>>(il2cpp_functions::resolve_icall(icall));
    if (it->second == nullptr) {
      PaperLogger.warn("Vivify could not resolve rendering setting getter '{}'", icall);
    }
  }
  return it->second;
}
...
template <typename T>
bool TryApplySetting(char const* icall, T value) {
  if (auto setter = ResolveSettingsSetter<T>(icall); setter != nullptr) {
    setter(value);
    return true;
  }
  return false;
...
template <typename T>
std::optional<T> TryReadSetting(char const* icall) {
  if (auto getter = ResolveSettingsGetter<T>(icall); getter != nullptr) {
    return getter();
  }
  return std::nullopt;
}
...
void RawApplyRenderSettingFloat(std::string const& name, float val) {
  if (name == "fogDensity") UnityEngine::RenderSettings::set_fogDensity(val);
    ...
    TryApplySetting<float>("UnityEngine.QualitySettings::set_shadowDistance", val);
  } else if (name == "shadowNearPlaneOffset") {
    TryApplySetting<float>("UnityEngine.QualitySettings::set_shadowNearPlaneOffset", val);
  }
}
...
void RawApplyRenderSettingBool(std::string const& name, bool val) {
  if (name == "fog") UnityEngine::RenderSettings::set_fog(val);
    ...
    TryApplySetting<bool>("UnityEngine.QualitySettings::set_realtimeReflectionProbes", val);
  } else if (name == "softParticles") {
    TryApplySetting<bool>("UnityEngine.QualitySettings::set_softParticles", val);
  } else if (name == "useOcclusionMesh") {
    TryApplySetting<bool>("UnityEngine.XR.XRSettings::set_useOcclusionMesh", val);
  }
}
void RawApplyRenderSettingInt(std::string const& name, int val) {
  if (name == "antiAliasing") UnityEngine::QualitySettings::set_antiAliasing(val);
    ...
    TryApplySetting<int>("UnityEngine.QualitySettings::set_anisotropicFiltering", val);
  else if (name == "pixelLightCount")
    TryApplySetting<int>("UnityEngine.QualitySettings::set_pixelLightCount", val);
  else if (name == "shadowCascades")
    TryApplySetting<int>("UnityEngine.QualitySettings::set_shadowCascades", val);
  else if (name == "shadowmaskMode")
    TryApplySetting<int>("UnityEngine.QualitySettings::set_shadowmaskMode", val);
  else if (name == "shadowProjection")
    TryApplySetting<int>("UnityEngine.QualitySettings::set_shadowProjection", val);
  else if (name == "shadowResolution")
    TryApplySetting<int>("UnityEngine.QualitySettings::set_shadowResolution", val);
  else if (name == "shadows")
    TryApplySetting<int>("UnityEngine.QualitySettings::set_shadows", val);
...
void Runtime::SaveRenderSetting(std::string const& name, RenderSettingKind kind) {
  ...
  if (name == "fog") saved.saved = UnityEngine::RenderSettings::get_fog();
    ...
    auto value = TryReadSetting<float>("UnityEngine.QualitySettings::get_shadowDistance");
    if (!value.has_value()) return;
    saved.saved = *value;
  }
  else if (name == "shadowNearPlaneOffset") {
    auto value = TryReadSetting<float>("UnityEngine.QualitySettings::get_shadowNearPlaneOffset");
    if (!value.has_value()) return;
    saved.saved = *value;
  }
  else if (name == "realtimeReflectionProbes") {
    auto value = TryReadSetting<bool>("UnityEngine.QualitySettings::get_realtimeReflectionProbes");
    if (!value.has_value()) return;
    saved.saved = *value;
  }
  else if (name == "softParticles") {
    auto value = TryReadSetting<bool>("UnityEngine.QualitySettings::get_softParticles");
    if (!value.has_value()) return;
    saved.saved = *value;
  }
  else if (name == "useOcclusionMesh") {
    auto value = TryReadSetting<bool>("UnityEngine.XR.XRSettings::get_useOcclusionMesh");
    if (!value.has_value()) return;
    saved.saved = *value;
  }
  else if (name == "anisotropicFiltering") {
    auto value = TryReadSetting<int>("UnityEngine.QualitySettings::get_anisotropicFiltering");
    if (!value.has_value()) return;
    saved.saved = *value;
  }
  else if (name == "pixelLightCount") {
    auto value = TryReadSetting<int>("UnityEngine.QualitySettings::get_pixelLightCount");
    if (!value.has_value()) return;
    saved.saved = *value;
  }
  else if (name == "shadowCascades") {
    auto value = TryReadSetting<int>("UnityEngine.QualitySettings::get_shadowCascades");
    if (!value.has_value()) return;
    saved.saved = *value;
  }
  else if (name == "shadowmaskMode") {
    auto value = TryReadSetting<int>("UnityEngine.QualitySettings::get_shadowmaskMode");
    if (!value.has_value()) return;
    saved.saved = *value;
  }
  else if (name == "shadowProjection") {
    auto value = TryReadSetting<int>("UnityEngine.QualitySettings::get_shadowProjection");
    if (!value.has_value()) return;
    saved.saved = *value;
  }
  else if (name == "shadowResolution") {
    auto value = TryReadSetting<int>("UnityEngine.QualitySettings::get_shadowResolution");
    if (!value.has_value()) return;
    saved.saved = *value;
  }
  else if (name == "shadows") {
    auto value = TryReadSetting<int>("UnityEngine.QualitySettings::get_shadows");
    if (!value.has_value()) return;
    saved.saved = *value;
  }
  else return;
