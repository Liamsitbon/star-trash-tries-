...
void CustomEventCallback(BeatmapCallbacksController* callbackController,
  ...
  PAPER_IL2CPP_CATCH_HANDLER(
      ...
      if (!callbackController || !customEventData || !customEventData->customData) return;
...
#define TYPE_GET(jsonName, varName)                                                                                    \
  ...
  if (!isType && typeHash == (jsonNameHash_##varName)) isType = true;
      ...
      // CJD you bugger
      if (!ad.parsed) {
        if (!callbackController->_beatmapData) return;
        auto customBeatmapDataOpt =
            il2cpp_utils::try_cast<CustomJSONData::CustomBeatmapData>(callbackController->_beatmapData);
        if (!customBeatmapDataOpt || !customBeatmapDataOpt.value()->customData) return;
        auto* customBeatmapData = customBeatmapDataOpt.value();
