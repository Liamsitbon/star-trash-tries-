...
static void RemoveCallback(TrackW track, ParentObject* object) {
  ...
  auto it = gameObjectModificationCallbacks.find(pair);
  if (it == gameObjectModificationCallbacks.end()) return;

  track.RemoveGameObjectCallback(it->second);
  gameObjectModificationCallbacks.erase(it);
}
...
void ParentObject::AssignTrack(ParentTrackEventData const& parentTrackEventData) {
  ...
  if (instance->track.v2) {
    ...
    parentTrackEventData.transformData.Apply(transform, NECaches::LeftHandedMode, false);

    std::array<TrackW, 1> _tracks = { parentTrackEventData.parentTrack };
    Tracks::GameObjectTrackController::HandleTrackData(
        parentGameObject, _tracks, StaticBeatmapObjectSpawnMovementData::kNoteLinesDistance,
...
void ParentController::OnDestroy() {
  ...
  // ResetRuntimeState can run before Unity destroys the ParentObject components.
  // Unregister callbacks while their owning TrackW is still known; simply
  // clearing the map would leave callbacks that capture stale ParentObject*.
  for (auto const& [pair, callback] : gameObjectModificationCallbacks) {
    TrackW track = pair.first;
    if (track) track.RemoveGameObjectCallback(callback);
  }
