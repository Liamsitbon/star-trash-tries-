...
MAKE_HOOK_MATCH(ObstacleController_Init, &ObstacleController::Init, void, ObstacleController* self,
  ...
  if (Hooks::isNoodleHookEnabled() && normalObstacleData != nullptr && NECaches::noodleMovementDataProviderPool) {
    auto provider = NECaches::noodleMovementDataProviderPool->get(normalObstacleData);
  ...
  if (!self || !normalObstacleData) return;
...
MAKE_HOOK_MATCH(ObstacleController_ManualUpdate, &ObstacleController::ManualUpdate, void, ObstacleController* self) {
  ...
  if (!self || !self->_obstacleData) return ObstacleController_ManualUpdate(self);
  ...
  if (!obstacleData->customData) return ObstacleController_ManualUpdate(self);
  ...
  if (offset.dissolve.has_value()) {
    ...
    if (!cutoutAnimationEffect) {
      ...
      if (obstacleCache.obstacleDissolve) {
    ...
    if (cutoutAnimationEffect) {
      ...
      static bool loggedMissingObstacleDissolve = false;
      if (!loggedMissingObstacleDissolve) {
        loggedMissingObstacleDissolve = true;
        NELogger::Logger.warn("Skipping obstacle dissolve: ObstacleDissolve/CutoutAnimateEffect is unavailable");
      }
    }
  }
...
MAKE_HOOK_MATCH(ObstacleController_GetPosForTime, &ObstacleController::GetPosForTime, Vector3, ObstacleController* self,
  ...
  if (!self || !self->_obstacleData) return ObstacleController_GetPosForTime(self, time);
  ...
  if (self->_obstacleData->klass != CustomKlass || !obstacleData->customData || !obstacleData->customData->value) {
    return ObstacleController_GetPosForTime(self, time);
  }
...
MAKE_HOOK_MATCH(ObstacleController_GetObstacleLength, &ObstacleController::GetObstacleLength, float,
  ...
  if (!self || !self->_obstacleData) return ObstacleController_GetObstacleLength(self);
  ...
  auto* obstacleData = reinterpret_cast<CustomJSONData::CustomObstacleData*>(self->_obstacleData);
  if (!obstacleData->customData || !obstacleData->customData->value) return ObstacleController_GetObstacleLength(self);
