...
MAKE_HOOK_MATCH(NoteJump_ManualUpdate, &NoteJump::ManualUpdate, Vector3, NoteJump* self) {
  ...
  // Beat Saber 1.40.8 stops refreshing these endpoints after the missed mark.
  // Noodle uses the variable provider for animated/track-driven note positions,
  // so long-lived notes (including Murder Plot and 42-flux effects) must keep
  // interpolating after that boundary just like current PC Noodle.
  if (self->_missedMarkReported) {
    self->_startPos = Sombrero::FastVector3(movement.moveEndPosition) +
                      Sombrero::FastVector3(self->_startOffset);
    self->_endPos = Sombrero::FastVector3(movement.jumpEndPosition) +
  ...
  auto vector2 = NEVector::Quaternion(self->_worldRotation) * NEVector::Vector3(self->_localPosition);
  if (noteUpdateAD) {
    auto offset = AnimationHelper::GetObjectOffset(noteUpdateAD->animationData, noteTracks, normalTime);
    if (offset.localPositionOffset.has_value()) {
      vector2 = vector2 + *offset.localPositionOffset;
    }
  }
  self->get_transform()->set_localPosition(vector2);
