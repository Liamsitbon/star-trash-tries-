...
static void UpdateMirror(Transform* mirroredNoteTransform, Transform* followedObjectTransform,
  ...
  auto* followedArrowCutoutEffect = followedDisappearingArrowController
                                        ? followedDisappearingArrowController->_arrowCutoutEffect.unsafePtr()
                                        : nullptr;
  auto followedDisappearingArrowControllerCutout =
      followedArrowCutoutEffect ? followedArrowCutoutEffect->_cutout : 0;
  ...
  if (il2cpp_functions::class_is_assignable_from(GameKlass, followedNote->klass) &&
    ...
    if (disappearingArrowController && followedDisappearingArrowController && followedArrowCutoutEffect)
      // SetArrowTransparency does 1 - x, so we resolve for x
      // 1 - x = y
      // -x = y - 1
      // x = -y + 1
      disappearingArrowController->SetArrowTransparency(
          (followedArrowCutoutEffect->_cutout - 1) * -1);
  }
}
