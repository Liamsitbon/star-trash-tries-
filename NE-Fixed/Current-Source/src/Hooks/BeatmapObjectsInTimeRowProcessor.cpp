...
MAKE_HOOK_MATCH(
  ...
  for (auto& [_, list] : notesInColumn) {
    for (int m = 0; m < list.size(); m++) {
      ...
      // V3 fake/interactable-free notes are often authored as a dense visual
      // stream: every note shares one logical column and is moved into its
      // actual position by definitePosition.  Assigning the normal ordinal
      // row here makes the spawn movement put hundreds of those notes on
      // different physical layers before their animation runs.  On Quest
      // that produces the "rows instead of one line" failure seen in Murder
