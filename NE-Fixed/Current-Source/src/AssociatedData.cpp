...
ObjectCustomData::ObjectCustomData(rapidjson::Value const& customData, CustomJSONData::CustomNoteData* noteData,
  ...
  // V3 fake arrays are uninteractable visual objects. Preserve that gameplay
  // contract even if an older loader failed to propagate the NE_fake marker:
  // such an object must never become a scoring note.
  if (!v2 && uninteractable.value_or(false) && !fake.value_or(false)) {
    fake = true;
  }

  if (noteData && (fake.value_or(false) || uninteractable.value_or(false))) {
