...
constexpr float GetJumpDuration(std::optional<float> inputNjs, std::optional<float> inputOffset) {
  ...
  // Per-object NJS/offset data uses BeatOffset semantics in PC Noodle. Do not
  // reinterpret a custom beat offset as a JumpDuration just because the
  // difficulty itself uses JumpDuration mode.
